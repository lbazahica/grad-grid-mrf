calc_beta <- function(beta, step = 0, min_step = 0.1, faktor=-4.37){
  ...
  if(step == 0){
    ...
    stepsize <- exp(faktor*var(esschain)/beta_g_var)
  }else if(step == 3){
...
dir_low <- function(beta, step = 0, min_step = 0.1, faktor = -4.37){
  matu_low <- matrix(, nrow = iter, ncol = 0)
  while(beta[1]!=0){
    beta_low <- calc_beta(beta[1], step, min_step, faktor)
...
dir_high <- function(beta, step = 0, min_step = 0.1, faktor=-4.37){
  matu_high <- matrix(, nrow = iter, ncol = 0)
  while(beta[length(beta)]!=2.5){
    beta_high <- calc_beta(beta[length(beta)], step, min_step, faktor)
